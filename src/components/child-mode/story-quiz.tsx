  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-purple-900/40 backdrop-blur-md">
      ...
      <motion.div
        ...
        className="bg-white rounded-[32px] md:rounded-[50px] p-6 md:p-12 shadow-2xl max-w-lg w-full text-center border-b-[12px] border-slate-200 relative overflow-hidden"
      >
        <AnimatePresence mode="wait">
          {!isFinished ? (
            ...
            >
              ...
              <h2 className="hidden md:block text-sm font-black text-purple-500 uppercase tracking-widest mb-2 mt-4 italic">Hikaye Testi</h2>
              <div className="flex justify-center gap-1 mb-3 md:mb-6 mt-4 md:mt-0">
                {questions.map((_, i) => (
                  <div 
                    key={i} 
                    className={cn(
              ...
              <div className="space-y-3 md:space-y-8">
                <div className="flex flex-col landscape:flex-row items-center justify-center gap-2 md:gap-4 px-2">
                  <button
                    onClick={playQuestion}
                    className="w-10 h-10 md:w-12 md:h-12 shrink-0 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center hover:bg-purple-200 transition-colors"
                  >
                    <Volume2 className={cn("w-5 h-5 md:w-6 md:h-6", isPlaying && "animate-pulse")} />
                  </button>
                  <h3 className="text-base md:text-2xl font-black text-slate-800 leading-tight">
                    {questions[currentStep].question}
                  </h3>
                </div>

                <div className="grid grid-cols-1 landscape:grid-cols-2 gap-2 md:gap-3">
                  {questions[currentStep].options.map((option, i) => (
                    <div key={i} className="relative group">
                      <button
                        onClick={() => handleOptionClick(i)}
                        className={cn(
                          "w-full p-3 pr-10 md:p-5 md:pr-14 rounded-[16px] md:rounded-[25px] text-sm md:text-lg font-bold transition-all border-b-4 flex items-center justify-between text-left",
                          selectedOption === null 
            ...
            >
              <div className="w-20 h-20 md:w-32 md:h-32 bg-yellow-400 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-8 shadow-[0_0_50px_rgba(251,191,36,0.5)] animate-bounce">
                <Trophy className="w-10 h-10 md:w-16 md:h-16 text-white" />
              </div>
              <h2 className="text-2xl md:text-4xl font-black text-slate-800 mb-2 md:mb-4 uppercase italic">TEBRİKLER!</h2>
              <p className="text-sm md:text-xl font-bold text-slate-500 mb-4 md:mb-8 px-4">
                {wrongAnswers === 0 
              ...
              <div className="flex justify-center gap-1 md:gap-2">
                {[...Array(5)].map((_, i) => (
                  <Sparkles key={i} className="w-6 h-6 md:w-8 md:h-8 text-yellow-400 animate-pulse" style={{ animationDelay: `${i * 0.2}s` }} />
                ))}
              </div>
            </motion.div>
          )}
