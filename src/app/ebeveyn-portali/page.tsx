    ...
    return (
        <div className="flex-1 w-full max-w-full space-y-4 sm:space-y-8 p-2 sm:p-8 pt-6 bg-muted/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <Card 
                  className={cn(
                    "p-6 bg-gradient-to-br from-purple-100 to-pink-100 border-purple-200 hover:shadow-lg transition-all group relative overflow-hidden",
                    user?.email === 'ibrahimcanonder_98@hotmail.com' ? "cursor-pointer" : "cursor-not-allowed"
                  )} 
                  onClick={() => {
                    if (user?.email === 'ibrahimcanonder_98@hotmail.com') {
                      router.push('/ebeveyn-portali/puan-merkezi');
                    } else {
                      toast({ title: 'Yakında Aktif!', description: 'Puan Merkezi yakında sizlerle olacak.' });
                    }
                  }}
                >
