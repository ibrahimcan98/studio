...
import { collection, query, where, addDoc, deleteDoc, getDocs, Timestamp, doc } from 'firebase/firestore';
...
export default function TakvimYonetimiPage() {
    ...
    const activeDays = useMemo(() => {
        ...
        return lessonSlots.map(slot => new Date(slot.startTime.seconds * 1000));
    }, [lessonSlots]);
    ...
    const slotsForSelectedDate = useMemo(() => {
        ...
        lessonSlots.forEach(slot => {
            ...
            if (slotDate >= start && slotDate < new Date(start.getTime() + 24 * 60 * 60 * 1000)) {
                const time = format(slotDate, 'HH:mm');
                slotsMap.set(time, { id: slot.id, status: slot.status, bookedBy: slot.bookedBy });
            }
        });
        return slotsMap;
    ...
    const handleTimeSlotClick = async (time: string) => {
        ...
        try {
            if (existingSlot) {
                ...
                toast({ title: 'Başarılı', description: 'Ders aralığı kaldırıldı.' });
            } else {
                // Slot doesn't exist, so create it
                await addDoc(collection(db, 'lesson-slots'), {
                    teacherId: user.uid,
                    startTime: Timestamp.fromDate(slotDateTime),
                    endTime: Timestamp.fromDate(new Date(slotDateTime.getTime() + 45 * 60 * 1000)), // Assuming 45 min lessons
                    status: 'available',
                    bookedBy: null,
                });
                toast({ title: 'Başarılı', description: 'Ders aralığı eklendi.', className: 'bg-green-500 text-white' });
            }
        } catch (error) {
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Ders vermek istediğiniz günleri ve saatleri seçin.
                    </p>
                </div>
            </div>
                        ...
                        {selectedDate ? (
                            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                                {timeSlots.map(time => {
                                    ...
                                    return (
                                        <Button
                                            ...
                                            disabled={isSubmitting[time] || isBooked}
                                        >
                             ...
                             <div className="flex items-center justify-center h-48 border-2 border-dashed rounded-lg bg-muted/50">
                                <p className="text-muted-foreground">Müsaitlik eklemek için bir tarih seçin.</p>
                            </div>
                        )}
                        <div className="flex flex-col gap-2 mt-6 text-sm text-muted-foreground">
                            <div className="flex items-center gap-2"><div className="w-4 h-4 rounded-full bg-primary"></div> Müsait</div>
                            <div className="flex items-center gap-2"><div className="w-4 h-4 rounded-full bg-destructive"></div> Rezerve Edilmiş</div>
                            <div className="flex items-center gap-2"><div className="w-4 h-4 rounded-full border bg-background"></div> Kapalı</div>
                        </div>
                    </div>
                </div>
            </Card>
