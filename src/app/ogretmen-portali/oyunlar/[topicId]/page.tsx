...
export default function DemoTopicPage() {
    ...
    const [isTreasureOpened, setIsTreasureOpened] = useState(false);
    ...
    const handleStageComplete = (currentStage: GameStage) => {
    ...
    if (!topic) {
        return (
            <div className="flex h-[calc(100vh-81px)] items-center justify-center bg-amber-50">
    ...
    if (stage === 'completed') {
        return (
            <div className="bg-sky-400 h-[calc(100vh-81px)] flex flex-col items-center justify-center p-8 text-center overflow-hidden">
    ...
    if (stage === 'map') {
        return (
            <div className={cn("bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] h-[calc(100vh-81px)] w-full relative flex flex-col items-center overflow-y-auto overflow-x-hidden scrollbar-hide transition-colors duration-1000", currentGradient)}>
                ...
                <div className="relative flex-1 w-full max-w-4xl flex items-center justify-center max-md:mt-8 max-md:mb-4 mt-12 mb-20 z-20 min-h-[400px] md:min-h-[700px] max-md:h-[75vh]">
                    ...
                    <div className="absolute left-[5%] md:left-[15%] top-[0%] flex flex-col items-center group cursor-pointer z-20" onClick={() => setStage('learning')}>
                        <div className="floating-island relative">
                            <div className={cn(
                                ...
                                completedStages.includes('learning') ? "bg-green-500 scale-110" : "bg-emerald-500"
                            )}>
                                {completedStages.includes('learning') ? <CheckCircle className="w-8 h-8 text-white" /> : "1"}
                            </div>
                    ...
                    {/* Level 2: Dinle & Bul */}
                    <div className="absolute right-[5%] md:right-[15%] top-[25%] flex flex-col items-center group cursor-pointer z-20" onClick={() => setStage('matching')}>
                        <div className="floating-island relative">
                            <div className={cn(
                                "absolute -top-4 -right-4 w-12 h-12 rounded-full border-4 border-white shadow-xl flex items-center justify-center z-30 font-black text-white text-2xl italic transition-all duration-500",
                                completedStages.includes('matching') ? "bg-green-500 scale-110" : "bg-blue-500"
                            )}>
                                {completedStages.includes('matching') ? <CheckCircle className="w-8 h-8 text-white" /> : "2"}
                            </div>
                            <div className="w-24 h-24 md:w-36 md:h-36 rounded-full bg-white border-[6px] md:border-[10px] border-blue-300 shadow-[0_15px_35px_rgba(0,0,0,0.3)] flex items-center justify-center transition-transform group-hover:scale-110 group-active:scale-95 z-10 relative overflow-hidden">
                        ...
                        <div className="mt-2 md:mt-4 bg-white px-6 py-2 md:px-8 md:py-3 rounded-full shadow-xl border-4 border-blue-100 group-hover:bg-blue-50 transition-colors">
                            <span className="font-bold text-blue-600 text-sm md:text-xl uppercase tracking-wider">Dinle & Bul</span>
                        </div>
                    </div>
                    ...
                    {/* Level 3: Bulmaca */}
                    <div className="absolute left-[5%] md:left-[15%] top-[50%] flex flex-col items-center group cursor-pointer z-20" onClick={() => setStage('quiz')}>
                        <div className="floating-island relative">
                            <div className={cn(
                                "absolute -top-4 -right-4 w-12 h-12 rounded-full border-4 border-white shadow-xl flex items-center justify-center z-30 font-black text-white text-2xl italic transition-all duration-500",
                                completedStages.includes('quiz') ? "bg-green-500 scale-110" : "bg-purple-500"
                            )}>
                                {completedStages.includes('quiz') ? <CheckCircle className="w-8 h-8 text-white" /> : "3"}
                            </div>
                            <div className="w-24 h-24 md:w-36 md:h-36 rounded-full bg-white border-[6px] md:border-[10px] border-purple-300 shadow-[0_15px_35px_rgba(0,0,0,0.3)] flex items-center justify-center transition-transform group-hover:scale-110 group-active:scale-95 z-10 relative overflow-hidden">
                        ...
                        <div className="mt-2 md:mt-4 bg-white px-6 py-2 md:px-8 md:py-3 rounded-full shadow-xl border-4 border-purple-100 group-hover:bg-purple-50 transition-colors">
                            <span className="font-bold text-purple-600 text-sm md:text-xl uppercase tracking-wider">Yapboz</span>
                        </div>
                    </div>
                        ...
                        {isTreasureOpened ? (
                            <div className="flex flex-col items-center animate-bounce">
                                <div className="relative w-28 h-28 md:w-36 md:h-36 rounded-[2rem] bg-white/40 backdrop-blur-md border-[6px] border-yellow-300 shadow-[0_0_40px_rgba(250,204,21,0.6)] flex items-center justify-center p-4">
                                    ...
                                    <Star className="w-16 h-16 text-yellow-500 drop-shadow-lg" />
                                </div>
    ...
    return (
        <div className="bg-[#7dd3fc] h-[calc(100vh-81px)] w-full flex overflow-hidden">
            {/* Sidebar Left Panel */}
            <aside className="w-16 md:w-32 shrink-0 bg-white/20 backdrop-blur-xl border-r border-white/40 flex flex-col items-center py-4 md:py-8 justify-between z-50 shadow-2xl">
                ...
                <div className="flex flex-col gap-6 md:gap-10 items-center">
                    {[
                        ...
                        const isCompleted = completedStages.includes(step.s);
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        isCurrent ? "bg-white text-sky-500 scale-125 shadow-[0_10px_20px_rgba(0,0,0,0.15)] border-white" :
                                            isCompleted ? "bg-green-400 text-white cursor-pointer hover:scale-110 border-green-600" : "bg-white/40 text-white cursor-pointer hover:bg-white/60 hover:scale-105 border-white/50"
                                    )}
                                >
                                    {step.label}
                                </button>
            ...
            <main className="flex-1 flex flex-col relative">
                ...
                {completedStages.includes(stage) && stage !== 'quiz' && (
                    <div className={cn("absolute left-0 right-0 flex justify-center z-40 pointer-events-none px-2", stage === 'matching' ? "bottom-8 md:bottom-12" : "top-8 md:bottom-12")}>
                        <Button
