...
import { Loader2, Trophy, Coins, Gem, Compass, BookOpen, Brain, MapPin, X } from 'lucide-react';
...
const FloatingElement = ({ children, className, style }: { children: React.ReactNode, className?: string, style?: React.CSSProperties }) => (
  <div
    className={cn("absolute opacity-20 animate-float-slow pointer-events-none z-0", className)}
    style={style}
  >
    {children}
  </div>
...
export default function TurkceHazinemPage() {
  ...
  const { data: childData, loading: childLoading } = useDoc(childDocRef);
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans relative">
      ...
      <div className="absolute inset-0 z-5 overflow-hidden pointer-events-none">
        <FloatingElement className="top-[5%] left-[10%] text-amber-600/20" style={{ animationDelay: '0s' }}>
          <Coins className="w-16 h-16" />
        </FloatingElement>
        <FloatingElement className="top-[15%] right-[15%] text-amber-600/20" style={{ animationDelay: '2s' }}>
          <Gem className="w-12 h-12" />
        </FloatingElement>
        <FloatingElement className="top-[30%] left-[20%] text-amber-600/15" style={{ animationDelay: '4s' }}>
          ...
          <div className="w-full flex justify-center pt-12 flex-shrink-0">
            <div className="text-center bg-white/90 p-6 rounded-[30px] shadow-lg border-4 border-amber-300 backdrop-blur-sm max-w-xl">
              <div className="inline-flex items-center gap-3 bg-amber-100/80 px-6 py-2 rounded-full border-2 border-amber-300 shadow-md mb-4">
                ...
                <span className="text-amber-800 font-black uppercase tracking-widest text-sm">Büyük Yaş Grubu</span>
              </div>
          ...
          {chests.map((chest, index) => (
            <div
              ...
              style={{
                ...
                '--mobile-left': chest.left === '65%' ? '55%' : '15%',
            ...
            >
              ...
              >
                {/* Sandık Numarası */}
                <div className={cn(
                  "absolute top-2 left-2 text-white w-8 h-8 rounded-full flex items-center justify-center font-black text-sm border-2 shadow-md z-20",
                  chest.isReview ? "bg-purple-600 border-purple-300 w-auto px-2 h-8" : "bg-amber-800 border-amber-200"
                )}>
                ...
                <div className="relative w-56 h-56 flex items-center justify-center">
                  <Image
                    src={chest.isLocked ? "/turkce-hazinem/sandik-kapali.png" : "/turkce-hazinem/sandik-acik.png"}
                    fill
                    className={cn("object-contain", chest.isReview && "scale-110 hue-rotate-15 drop-shadow-[0_0_15px_rgba(168,85,247,0.4)]")}
                    alt={chest.title}
                  />
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="w-24 h-24 mb-6 relative">
                <Image src="/turkce-hazinem/sandik-kapali.png" fill className="object-contain" alt="Kilitli" />
              </div>
      ...
      <style jsx global>{`
        ...
        @keyframes float {
          0%, 100% { transform: translateY(0px) translateX(-50%) rotate(0deg); }
